...
# remove blank images and misfires
ud_dat$Blank[ud_dat$Blank == "f"] <- FALSE
ud_dat <- filter(ud_dat, Blank == FALSE)
ud_dat <- filter(ud_dat, Species != "No animal")
...
# Try to parse Date_Time.Captured, flag bad ones
ud_dat <- ud_dat %>%
  mutate(
    Date_Time.Captured = suppressWarnings(
      ymd_hms(Date_Time.Captured, truncated = 2, tz = "UTC")
    )
  )
...
if(nrow(bad_dates) > 0){
  message("Warning: ", nrow(bad_dates), " rows had invalid Date_Time.Captured values.")
}
